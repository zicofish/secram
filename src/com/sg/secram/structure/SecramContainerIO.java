...
public class SecramContainerIO {
     ...
     */
    public static boolean readContainerHeader(final SecramContainer container, final InputStream inputStream) throws IOException {
    ...
    public static SecramContainer readContainer(final InputStream inputStream) throws IOException {
        ...
        for(int i = 1; i < container.blockCount; i++){
        	...
        	switch(block.getContentType()){
        		...
        		case EXTERNAL:
        			...
        			break;
        		default:
        			throw new RuntimeException("Not a content block, content type id " + block.getContentType().name());
        	}
        }
     ...
     * @throws IOException as per java IO contract
     */
    public static int writeContainerHeader(final SecramContainer container, final OutputStream outputStream) throws IOException {

        int length = (CramInt.writeInt32(container.containerByteSize, outputStream) + 7) / 8;
        length += (ITF8.writeUnsignedITF8(container.containerID, outputStream) + 7) / 8;
        length += (LTF8.writeUnsignedLTF8(container.containerSalt, outputStream) + 7) / 8;
        length += (LTF8.writeUnsignedLTF8(container.absolutePosStart, outputStream) + 7) / 8;
        length += (LTF8.writeUnsignedLTF8(container.absolutePosEnd, outputStream) + 7) / 8;
        length += (ITF8.writeUnsignedITF8(container.coverageStart, outputStream) + 7) / 8;
        length += (ITF8.writeUnsignedITF8(container.qualityLenStart, outputStream) + 7) / 8;
        length += (ITF8.writeUnsignedITF8(container.nofRecords, outputStream) + 7) / 8;
        length += (LTF8.writeUnsignedLTF8(container.globalRecordCounter, outputStream) + 7) / 8;
     ...
     * @throws IOException as per java IO contract
     */
    public static int writeContainer(final SecramContainer container, final OutputStream outputStream) throws IOException {
