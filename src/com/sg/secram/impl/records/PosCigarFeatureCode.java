...
public enum PosCigarFeatureCode {
	...
	I('I', 'I', "Insertion after position", true, true, false),
	S('S', 'S', "Soft clipping after position", true, true, false),
	H('H', 'H', "Hard clipping after position", true, false, false),
	P('P', 'P', "Padding after position", false, false, false);
	
	
	
	private PosCigarFeatureCode(char character, char bamCharacter, String fullName, boolean hasLength, boolean hasBases, boolean consumesRef) {
		c = character;
	...
	public static PosCigarFeatureCode getFeatureCode(CigarOperator op, boolean starting, boolean match){
		switch(op){
			case I:
				if(starting) return F;
				return I;
			case S:
				if(starting) return R;
				return S;
			case H:
				if(starting) return G;
				return H;
			case P:
				if(starting) return O;
				return P;
			case D:
				return D;
			case N:
