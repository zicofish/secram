 ...
 * jrobinso
 * <p/>
 * the feature reader class, which uses indices and codecs to read in Tribble
 * file formats.
 */
public abstract class AbstractFeatureReader<T extends Feature, SOURCE>
	...
	public static final Set<String> BLOCK_COMPRESSED_EXTENSIONS = Collections
			.unmodifiableSet(new HashSet<String>(Arrays.asList(".gz", ".gzip",
					".bgz", ".bgzf")));

	/**
	 * Calls {@link #getFeatureReader(String, FeatureCodec, boolean)} with
	 * {@code requireIndex} = true
	 */
	...
	/**
	 * {@link #getFeatureReader(String, String, FeatureCodec, boolean)} with
	 * {@code null} for indexResource
	 * 
	 * @throws TribbleException
	 */
	public static <FEATURE extends Feature, SOURCE> AbstractFeatureReader<FEATURE, SOURCE> getFeatureReader(
			...
			final boolean requireIndex) throws TribbleException {
	 ...
	 *            the feature file to create from
	 * @param indexResource
	 *            the index for the feature file. If null, will auto-generate
	 *            (if necessary)
	 * @param codec
	 * @param requireIndex
	 *            whether an index is required for this file
	 * @return
	 * @throws TribbleException
	 */
	public static <FEATURE extends Feature, SOURCE> AbstractFeatureReader<FEATURE, SOURCE> getFeatureReader(
		...
		try {
			...
			if (methods.isTabix(featureResource, indexResource)) {
				...
				return new TabixFeatureReader<FEATURE, SOURCE>(featureResource,
						indexResource, (AsciiFeatureCodec) codec);
			}
			// Not tabix => tribble index file (might be gzipped, but not block
			// gzipped)
			else {
				return new TribbleIndexedFeatureReader<FEATURE, SOURCE>(
						featureResource, indexResource, codec, requireIndex);
			}
		} catch (IOException e) {
			throw new TribbleException.MalformedFeatureFile(
					"Unable to create BasicFeatureReader using feature file ",
					featureResource, e);
	 ...
	 *
	 * @param featureResource
	 *            the path to the source file containing the features
	 * @param codec
	 *            used to decode the features
	 * @param index
	 *            index of featureResource
	 * @return a reader for this data
	 * @throws TribbleException
	 */
	public static <FEATURE extends Feature, SOURCE> AbstractFeatureReader<FEATURE, SOURCE> getFeatureReader(
			final String featureResource,
			final FeatureCodec<FEATURE, SOURCE> codec, final Index index)
			throws TribbleException {
		try {
			return new TribbleIndexedFeatureReader<FEATURE, SOURCE>(
					featureResource, codec, index);
		} catch (IOException e) {
			throw new TribbleException.MalformedFeatureFile(
					"Unable to create AbstractFeatureReader using feature file ",
					featureResource, e);
		}

	}

	protected AbstractFeatureReader(final String path,
	...
	/**
	 * Whether the reader has an index or not Default implementation returns
	 * false
	 * 
	 * @return
	 */
	...
	static class EmptyIterator<T extends Feature> implements
	...
	public static class ComponentMethods {
		...
		public boolean isTabix(String resourcePath, String indexPath)
				throws IOException {
			if (indexPath == null) {
				indexPath = ParsingUtils.appendToPath(resourcePath,
