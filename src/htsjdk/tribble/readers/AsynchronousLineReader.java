...
/**
 * A LineReader implementation that delegates the work of reading and fetching
 * lines to another thread. The thread terminates when it encounters EOF in the
 * underlying reader, or when this LineReader is closed.
 *
 * @author mccowan
 */
public class AsynchronousLineReader implements LineReader {
	...
	public String readLine() {
		try {
			// Continually poll until we get a result, unless the underlying
			// reader is finished.
			for (;;) {
				checkAndThrowIfWorkerException();
				final String pollResult = this.lineQueue.poll(100,
						TimeUnit.MILLISECONDS); // Not ideal for small files.
				if (pollResult == null) {
					if (eofReached) {
						checkAndThrowIfWorkerException();
						return lineQueue.poll(); // If there is nothing left,
													// returns null as expected.
													// Otherwise, grabs next
													// element.
					}
				} else {
					return pollResult;
	...
	private void checkAndThrowIfWorkerException() {
		if (workerException != null) {
			throw new TribbleException(
					"Exception encountered in worker thread.", workerException);
		}
	}
				...
				for (;;) {
					...
					if (line == null) {
						...
						try {
							...
							/**
							 * A thread interruption is not an exceptional
							 * state: it means a
							 * {@link AsynchronousLineReader#close();} has been
							 * called, so shut down gracefully.
							 */
							break;
						}
					}
