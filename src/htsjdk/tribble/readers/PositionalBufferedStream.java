...
/**
 * A wrapper around an {@code InputStream} which performs it's own buffering,
 * and keeps track of the position.
 * 
 * @author depristo
 */
public final class PositionalBufferedStream extends InputStream implements
		Positional {
	...
	@Override
	public final int read(final byte[] bytes, final int start, final int len)
			throws IOException {
		if (len == 0) // If len is zero, then no bytes are read and 0 is
						// returned
			return 0;
		if (nChars < 0) // If no byte is available because the stream is at end
			...
			while (remaining > 0) {
				...
				// we copy as many bytes from the buffer as possible, up to the
				// number of need
				final int nCharsToCopy = Math.min(nChars - nextChar, remaining);
				System.arraycopy(buffer, nextChar, bytes, start + nRead,
						nCharsToCopy);

				// update nextChar (pointer into buffer) and keep track of nRead
				// and remaining
			...
			// make sure we update our position tracker to reflect having
			// advanced by nRead bytes
			position += nRead;

			/**
			 * Conform to {@link InputStream#read(byte[], int, int)} contract by
			 * returning -1 if EOF and no data was read.
			 */
	...
	private static void readFileByLine(final String name, final InputStream is)
			throws IOException {
	...
	private static void readFileInChunks(final String name, final InputStream is)
			throws IOException {
		final long t0 = System.currentTimeMillis();
		long chunk = 0;
	...
	private static final void printStatus(final String name, long lineCount,
			double rate, long dt) {
		System.out
				.printf("%30s: %d lines read.  Rate = %.2e lines per second.  DT = %d%n",
