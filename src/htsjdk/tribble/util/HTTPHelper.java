...
/**
 * Simple implementation of URLHelper based on the JDK URL and HttpURLConnection
 * classes. This version optionally takes a proxy, but does not support
 * authentication.
 *
 * @author jrobinso
 * @date Jun 28, 2011
...
public class HTTPHelper implements URLHelper {
		...
		try {
			...
			if ((con.getResponseCode() != HttpURLConnection.HTTP_OK)) {
				System.out.println("Error (" + con.getResponseMessage()
						+ " ) fetching content length: " + url);
				return -1;
			} else {
				String contentLength = con.getHeaderField("Content-Length");
				return contentLength == null ? -1 : Long
	...
	/**
	 * Open an input stream for the requested byte range. Its the client's
	 * responsibility to close the stream.
	 *
	 * @param start
	 *            start of range in bytes
	 * @param end
	 *            end of range ni bytes
	 * @return
	 * @throws IOException
	 */
	@Override
	...
	class WrapperInputStream extends FilterInputStream {
		...
		protected WrapperInputStream(HttpURLConnection connection,
