...
/**
 * An implementation of an interval tree, following the explanation. from CLR.
 * For efficiently finding all intervals which overlap a given interval or
 * point.
 * <p/>
 * References: http://en.wikipedia.org/wiki/Interval_tree
 * <p/>
 * Cormen, Thomas H.; Leiserson, Charles E., Rivest, Ronald L. (1990).
 * Introduction to Algorithms (1st ed.). MIT Press and McGraw-Hill. ISBN
 * 0-262-03141-8
 */
public class IntervalTree {
	...
	/**
	 * The estimated size of the tree. We keep a running count on each insert,
	 * this getter returns that count.
	 *
	 * @return
	 * @see #size()
	 */
	...
	private List<Interval> searchAll(Interval interval, Node node,
	...
	private void update(Node node) {
		node.max = Math.max(Math.max(node.left.max, node.right.max),
				node.interval.end);
		node.min = Math.min(Math.min(node.left.min, node.right.min),
				node.interval.start);
	}
	...
	private boolean allRedNodesFollowConstraints(Node node) {
		...
		return (node.left.color == Node.BLACK && node.right.color == Node.BLACK
	...
	static class Node {
		...
		// Color and parent are used for inserts. If tree is immutable these are
