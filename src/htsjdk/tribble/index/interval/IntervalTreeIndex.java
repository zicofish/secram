...
public class IntervalTreeIndex extends AbstractIndex {
	 ...
	 *            as appropriate. Caller should close after this object is
	 *            constructed.
	 */
	...
	public static class ChrIndex implements htsjdk.tribble.index.ChrIndex {
		...
		public List<Block> getBlocks(final int start, final int end) {
			...
			final List<Interval> intervals = tree.findOverlapping(new Interval(
					start, end));

			// save time (and save throwing an exception) if the blocks are
			// empty, return now
			...
			Arrays.sort(blocks, new Comparator<Block>() {
				...
				public int compare(final Block b1, final Block b2) {
					// this is a little cryptic because the normal method
					// (b1.getStartPosition() - b2.getStartPosition()) wraps in
					// int space and we incorrectly sort the blocks in extreme
					// cases
					return b1.getStartPosition() - b2.getStartPosition() < 1 ? -1
							: (b1.getStartPosition() - b2.getStartPosition() > 1 ? 1
									: 0);
				}
			});

			// Consolidate blocks that are close together
			final List<Block> consolidatedBlocks = new ArrayList<Block>(
					blocks.length);
		...
		public void read(final LittleEndianInputStream dis) throws IOException {
			...
			while (nIntervals-- > 0) {
				...
				final Interval iv = new Interval(start, end, new Block(pos,
						size));
				tree.insert(iv);
			}
