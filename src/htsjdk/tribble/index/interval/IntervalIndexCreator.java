...
public class IntervalIndexCreator extends TribbleIndexCreator {
	...
	/**
	 * Instance variable for the number of features we currently are storing in
	 * the interval
	 */
	...
	public void addFeature(final Feature feature, final long filePosition) {
		// if we don't have a chrIndex yet, or if the last one was for the
		// previous contig, create a new one
		if (chrList.size() == 0
				|| !chrList.getLast().getName().equals(feature.getChr())) {
			// if we're creating a new chrIndex (not the first), make sure to
			// dump the intervals to the old chrIndex
			if (chrList.size() != 0)
		...
		if (featureCount >= featuresPerInterval || intervals.size() == 0) {
			...
			if (intervals.size() > 0)
				intervals.get(intervals.size() - 1).setEndFilePosition(
						filePosition);
		...
		intervals.get(intervals.size() - 1).setStop(
				Math.max(feature.getEnd(), intervals.get(intervals.size() - 1)
						.getStop()));
		featureCount++;
	}
	 ...
	 * 
	 * @param currentPos
	 *            the current position, for the last entry in the interval list
	 */
	...
	/**
	 * finalize the index; create a tree index given the feature list passed in
	 * so far
	 * 
	 * @param finalFilePosition
	 *            the final file position, for indexes that have to close out
	 *            with the final position
	 * @return a Tree Index
	 */
	@Override
	public Index finalizeIndex(final long finalFilePosition) {
		final IntervalTreeIndex featureIndex = new IntervalTreeIndex(
...
/**
 * The interval class isn't mutable; use this private class as a temporary
 * storage until we're ready to make intervals
 */
class MutableInterval {
	...
	public Interval toInterval() {
		return new Interval(start, stop, new Block(startFilePosition,
				endFilePosition - startFilePosition));
	}
