...
/**
 * <p/>
 * An abstract implementation of the index class. This class takes care of the
 * basics that are common to all of the current indexing classes; including the
 * version information, common header properties, and reading and writing the
 * header to disk.
 *
 * @author jrobinso
 */
public abstract class AbstractIndex implements MutableIndex {
	...
	public static final int MAGIC_NUMBER = 1480870228; // byte[]{'T', 'I', 'D',
														// 'X'};
	...
	protected long indexedFileSize = NO_FILE_SIZE; // The size of the indexed
													// file
	protected long indexedFileTS = NO_TS; // The timestamp
	protected String indexedFileMD5 = NO_MD5; // The MD5 value, generally not
												// filled in (expensive to calc)
	protected int flags;
	 ...
	 */
	private static final int SEQUENCE_DICTIONARY_FLAG = 0x8000; // if we have a
																// sequence
																// dictionary in
																// our header
	...
	/**
	 * @param obj
	 * @return true if this and obj are 'effectively' equivalent data
	 *         structures.
	 */
	@Override
	public boolean equalsIgnoreProperties(final Object obj) {
		...
		if (version != other.version) {
			System.err.printf("equals version: this %d != other %d%n", version,
					other.version);
			return false;
		}

		if (indexedFile != other.indexedFile
				&& (indexedFile == null || !indexedFile
						.equals(other.indexedFile))) {
			System.err.printf("equals indexedFile: this %s != other %s%n",
					indexedFile, other.indexedFile);
			return false;
		}

		if (indexedFileSize != other.indexedFileSize) {
			System.err.printf("equals indexedFileSize: this %d != other %d%n",
					indexedFileSize, other.indexedFileSize);
			return false;
		}

		if (!indexedFileMD5.equals(other.indexedFileMD5)) {
			System.err.printf("equals indexedFileMD5: this %s != other %s%n",
					indexedFileMD5, other.indexedFileMD5);
			return false;
		}

		if (flags != other.flags) {
			System.err.printf("equals flags: this %d != other %d%n", flags,
					other.flags);
			return false;
		}

		if (!chrIndices.equals(other.chrIndices)) {
			System.err.printf("equals chrIndeces: this %s != other %s%n",
					chrIndices, other.chrIndices);
			return false;
		}
	...
	/**
	 * create an abstract index, with defaults for the version value, and empty
	 * properties and chromosome lists
	 */
	...
	public AbstractIndex(final AbstractIndex parent) {
		...
		this.properties = (LinkedHashMap<String, String>) parent.properties
				.clone();
	}

	protected void validateIndexHeader(final int indexType,
		...
		if (magicNumber != MAGIC_NUMBER) {
			throw new TribbleException(String.format(
					"Unexpected magic number %d", magicNumber));
		}
		final int type = dis.readInt();
		if (type != indexType) {
			throw new TribbleException(String.format(
					"Unexpected index type %d", type));
		}
	...
	public void finalizeIndex() {
		// these two functions must be called now because the file may be being
		// written during on the fly indexing
		if (indexedFile != null) {
	 ...
	 *
	 * @param dos
	 *            the little endian output stream
	 * @throws IOException
	 *             an exception when we can't write to the file
	 */
	private void writeHeader(final LittleEndianOutputStream dos)
			throws IOException {
	 ...
	 * @throws IOException
	 *             if we fail to read from the file at any point
	 */
	private void readHeader(final LittleEndianInputStream dis)
			throws IOException {
	...
	/**
	 * Kept to maintain backward compatibility with pre version 3 indexes. The
	 * sequence dictionary is no longer used, use getSequenceNames() instead.
	 *
	 * @param dis
	 * @throws IOException
	 */
	private void readSequenceDictionary(final LittleEndianInputStream dis)
			throws IOException {
		final int size = dis.readInt();
		if (size < 0)
			throw new IllegalStateException(
					"Size of the sequence dictionary entries is negative");
	...
	public void writeBasedOnFeatureFile(final File featureFile)
		...
		if (!featureFile.isFile())
			return;
		final LittleEndianOutputStream idxStream = new LittleEndianOutputStream(
	...
	public void read(final LittleEndianInputStream dis) throws IOException {
		try {
			...
			while (nChromosomes-- > 0) {
				final ChrIndex chrIdx = (ChrIndex) getChrIndexClass()
						.newInstance();
				chrIdx.read(dis);
	...
	protected void printIndexInfo() {
		System.out.println(String.format("Index for %s with %d indices",
				indexedFile, chrIndices.size()));
		final BlockStats stats = getBlockStats(true);
		System.out.println(String.format("  total blocks %d", stats.total));
		System.out.println(String
				.format("  total empty blocks %d", stats.empty));
	}
			...
			if (blocks != null) {
				...
				if (logDetails)
					System.out.println(String.format(
							"  %s => %d blocks, %d empty, %.2f", elt.getKey(),
							nBlocks, nEmptyBlocks, (100.0 * nEmptyBlocks)
									/ nBlocks));
			}
		}
	...
	protected String statsSummary() {
		...
		return String.format("%12d blocks (%12d empty (%.2f%%))", stats.total,
				stats.empty, (100.0 * stats.empty) / stats.total);
	}
