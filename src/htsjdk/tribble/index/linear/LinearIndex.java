...
/**
 * Index defined by dividing the genome by chromosome, then each chromosome into
 * bins of fixed width (in genomic coordinates). Features are allocated to bins
 * by start position. The longest feature in each recorded and used to adjust
 * the start position of a query to include all bins that might have a feature
 * that overlaps the query interval. This works well for feature sets of
 * approximately homogeneous length, or whose longest feature is on the order of
 * the bin width or less.
 * <p/>
 * magicNumber integer type integer version integer filename null terminated
 * character array filesize long lastModified long md5 String flags integer
 * <p/>
...
public class LinearIndex extends AbstractIndex {
	...
	public static final double MAX_FEATURES_PER_BIN = Double.valueOf(System
			.getProperty("MAX_FEATURES_PER_BIN", "100"));
	public static final int INDEX_TYPE = IndexType.LINEAR.fileHeaderTypeIdentifier;

	private final static int MAX_BIN_WIDTH = 1 * 1000 * 1000 * 1000; // widths
																		// must
																		// be
																		// less
																		// than
	...
	// 1MB: we will no merge bins with any features in them beyond this size, no
	// matter how sparse, per chromosome
	private static final long MAX_BIN_WIDTH_FOR_OCCUPIED_CHR_INDEX = Long
			.valueOf(System.getProperty("MAX_BIN_WIDTH_FOR_OCCUPIED_CHR_INDEX",
					"1024000"));
	 ...
	 * 
	 * @param inputStream
	 *            This method assumes that the input stream is already buffered
	 *            as appropriate.
	 */
	public LinearIndex(final InputStream inputStream) throws IOException {
		final LittleEndianInputStream dis = new LittleEndianInputStream(
				inputStream);
	...
	public boolean isCurrentVersion() {
		...
		// todo fixme nasty hack to determine if this is an old style V3 linear
		// index (without nFeaturesPerBin)
	...
	public List<String> getSequenceNames() {
		return (chrIndices == null ? Collections.EMPTY_LIST : Collections
				.unmodifiableList(new ArrayList<String>(chrIndices.keySet())));
	}
	 ...
	 * <p/>
	 * There's a constant bin width, so that each block corresponds to a
	 * specific interval over the genome based on its index, as in:
	 * <p/>
	 * Block 0: (0 - binWidth] Block 1: (binWidth - 2 * binWidth] Block 2: (2 *
	 * binWidth - 3 * binWidth]
	 * <p/>
	...
	public static class ChrIndex implements htsjdk.tribble.index.ChrIndex {
		...
		public List<Block> getBlocks(final int start, final int end) {
			if (blocks.isEmpty()) {
				...
				// Adjust position for the longest feature in this chromosome.
				// This insures we get
				// features that start before the bin but extend into it
				final int adjustedPosition = Math
						.max(start - longestFeature, 0);
				final int startBinNumber = adjustedPosition / binWidth;
				if (startBinNumber >= blocks.size()) // are we off the end of
														// the bin list, so
														// return nothing
					return Collections.emptyList();
				else {
					final int endBinNumber = Math.min((end - 1) / binWidth,
							blocks.size() - 1);

					// By definition blocks are adjacent for the liner index.
					// Combine them into one merged block

					final long startPos = blocks.get(startBinNumber)
							.getStartPosition();
					final long endPos = blocks.get(endBinNumber)
							.getStartPosition()
		...
		@Override
		public void write(final LittleEndianOutputStream dos)
				throws IOException {
		...
		public void read(final LittleEndianInputStream dis) throws IOException {
			...
			// largestBlockSize and totalBlockSize are old V3 index values.
			// largest block size should be 0 for
			// all newer V3 block. This is a nasty hack that should be removed
			// when we go to V4 (XML!) indices
			OLD_V3_INDEX = dis.readInt() > 0;
			nFeatures = dis.readInt();

			// note the code below accounts for > 60% of the total time to read
			// an index
		...
		public boolean equals(final Object obj) {
			...
			return binWidth == other.binWidth
					...
					&& nFeatures == other.nFeatures && name.equals(other.name)
		...
		private static boolean badBinWidth(final ChrIndex idx) {
			if (idx.binWidth > MAX_BIN_WIDTH || idx.binWidth < 0) // an overflow
																	// occurred
				return true;
		...
		private static ChrIndex optimize(ChrIndex idx, final double threshold,
				int level) {
			ChrIndex best = idx;
			...
			while (true) {
				...
				if (score > threshold || idx.getNBlocks() == 1
						|| badBinWidth(idx))
					break;
				else {
		...
		private static ChrIndex mergeBlocks(final ChrIndex idx) {
			final ChrIndex merged = new ChrIndex(idx.name, idx.binWidth * 2); // increasing
																				// width
																				// by
																				// 2
																				// each
			...
			if (!blocks.hasNext())
				throw new IllegalStateException(
						"Block iterator cannot be empty at the start for "
								+ idx.getName());

			// extremely simple merging algorithm. Walk left to right, joining
			// up blocks adjacent blocks.
			while (blocks.hasNext()) {
				...
				if (b2 == null)
					...
					// the new block is simply the start of the first block and
					// the size of both together
					merged.addBlock(new Block(b1.getStartPosition(), b1
							.getSize() + b2.getSize()));
			}
	 ...
	 * 
	 * @param threshold
	 *            threshold to use for optimizing each constituent
	 *            {@code chrIndex}
	...
	public Index optimize(final double threshold) {
		if (enableAdaptiveIndexing) {
			...
			final List<ChrIndex> newIndices = new ArrayList<ChrIndex>(
					this.chrIndices.size());
			for (final String name : chrIndices.keySet()) {
				final LinearIndex.ChrIndex oldIdx = (LinearIndex.ChrIndex) chrIndices
						.get(name);
	...
	public void writeTable(final PrintStream out) {
		...
		for (final String name : chrIndices.keySet()) {
			final LinearIndex.ChrIndex chrIdx = (LinearIndex.ChrIndex) chrIndices
					.get(name);
			int blockCount = 0;
			for (final Block b : chrIdx.getBlocks()) {
				out.printf("%s %d %.2f %d %d %d %d %d%n", name,
						chrIdx.binWidth, chrIdx.getAverageFeatureSize(),
						chrIdx.getNFeatures(), blockCount, blockCount
								* chrIdx.binWidth, b.getSize(),
