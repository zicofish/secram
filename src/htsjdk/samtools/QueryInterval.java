...
public class QueryInterval implements Comparable<QueryInterval> {
	...
	/**
	 * Index of reference sequence, based on the sequence dictionary of the BAM
	 * file being queried.
	 */
	...
	/**
	 * 1-based, inclusive. If <= 0, implies that the interval goes to the end of
	 * the reference sequence
	 */
	public final int end;

	public QueryInterval(final int referenceIndex, final int start,
			final int end) {
		if (referenceIndex < 0) {
			throw new IllegalArgumentException("Invalid reference index "
					+ referenceIndex);
		}
	 ...
	 */
	public static QueryInterval[] optimizeIntervals(
			final QueryInterval[] inputIntervals) {
		...
		for (int i = 1; i < inputIntervals.length; ++i) {
			...
			if (previous.abuts(next) || previous.overlaps(next)) {
				final int newEnd = ((previous.end == 0 || next.end == 0) ? 0
						: Math.max(previous.end, next.end));
				previous = new QueryInterval(previous.referenceIndex,
						previous.start, newEnd);
			} else {
				unique.add(previous);
				previous = next;
			}
