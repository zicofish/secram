...
/**
 * Describes functionality for objects that produce {@link SAMRecord}s and
 * associated information.
 *
 * Currently, only deprecated readers implement this directly; actual readers
 * implement this via {@link ReaderImplementation} and
 * {@link PrimitiveSamReader}, which {@link SamReaderFactory} converts into full
 * readers by using {@link PrimitiveSamReaderToSamReaderAdapter}.
 *
 * @author mccowan
 */
public interface SamReader extends Iterable<SAMRecord>, Closeable {
	...
	public abstract class Type {
		...
		/**
		 * The recommended file extension for SAMs of this type, without a
		 * period.
		 */
		public abstract String fileExtension();

		/**
		 * The recommended file extension for SAM indexes of this type, without
		 * a period, or null if this type is not associated with indexes.
		 */
		...
		static class TypeImpl extends Type {
			...
			TypeImpl(final String name, final String fileExtension,
			...
			public String toString() {
				return String
						.format("TypeImpl{name='%s', fileExtension='%s', indexExtension='%s'}",
	...
	public interface Indexing {
		/**
		 * Retrieves the index for the given file type. Ensure that the index is
		...
		/**
		 * Returns true if the supported index is browseable, meaning the bins
		 * in it can be traversed and chunk data inspected and retrieved.
		 *
	...
	/**
	 * @return the {@link htsjdk.samtools.SamReader.Type} of this
	 *         {@link htsjdk.samtools.SamReader}
	 */
	public Type type();

	/**
	 * @return a human readable description of the resource backing this sam
	 *         reader
	 */
	 ...
	 *             If {@link #hasIndex()} returns false.
	 */
	 ...
	 * one left off. For a BAM constructed from a SeekableStream or File, each
	 * new iteration starts at the first record.
	 * <p/>
	 ...
	 * @param contained
	 *            If true, each SAMRecord returned will have its alignment
	...
	/**
	 * Iterate over records that match one of the given intervals. This may be
	 * more efficient than querying each interval separately, because multiple
	 * reads of the same SAMRecords is avoided.
	 * <p/>
	 ...
	 * Note that indexed lookup is not perfectly efficient in terms of disk I/O.
	 * I.e. some SAMRecords may be read and then discarded because they do not
	 * match an interval of interest.
	 * <p/>
	 * Note that an unmapped read will be returned by this call if it has a
	 * coordinate for the purpose of sorting that is in the query region.
	 *
	 * @param intervals
	 *            Intervals to be queried. The intervals must be optimized, i.e.
	 *            in order, with overlapping and abutting intervals merged. This
	 *            can be done with
	 *            {@link htsjdk.samtools.QueryInterval#optimizeIntervals}
	 * @param contained
	 *            If true, each SAMRecord returned is will have its alignment
	 *            completely contained in one of the intervals of interest. If
	 *            false, the alignment of the returned SAMRecords need only
	 *            overlap one of the intervals of interest.
	 * @return Iterator over the SAMRecords matching the interval.
	 */
	public SAMRecordIterator query(final QueryInterval[] intervals,
			final boolean contained);

	/**
	 * Iterate over records that overlap any of the given intervals. This may be
	 * more efficient than querying each interval separately, because multiple
	 * reads of the same SAMRecords is avoided.
	 * <p/>
	 * Only valid to call this if hasIndex() == true.
	...
	/**
	 * Iterate over records that are contained in the given interval. This may
	 * be more efficient than querying each interval separately, because
	 * multiple reads of the same SAMRecords is avoided.
	 * <p/>
	 * Only valid to call this if hasIndex() == true.
	 * <p/>
	 * Only a single open iterator on a given SAMFileReader may be extant at any
	 ...
	 * <p/>
	 * Note that it is not possible to call queryMate when iterating over the
	 * SAMFileReader, because queryMate requires its own iteration, and there
	 * cannot be two simultaneous iterations on the same SAMFileReader. The
	 * work-around is to open a second SAMFileReader on the same input file, and
	 * call queryMate on the second reader.
	 *
	 * @param rec
	...
	/**
	 * The minimal subset of functionality needed for a {@link SAMRecord} data
	 * source. {@link SamReader} itself is somewhat large and bulky, but the
	 * core functionality can be captured in relatively few methods, which are
	 * included here. For documentation, see the corresponding methods in
	 * {@link SamReader}.
	 *
	 * See also: {@link PrimitiveSamReaderToSamReaderAdapter},
	 * {@link ReaderImplementation}
	 *
	 */
	public interface PrimitiveSamReader {
		...
		CloseableIterator<SAMRecord> queryAlignmentStart(String sequence,
				int start);
	...
	/**
	 * Decorator for a {@link SamReader.PrimitiveSamReader} that expands its
	 * functionality into a {@link SamReader}, given the backing
	 * {@link SamInputResource}.
	 *
	 * Wraps the {@link Indexing} interface as well, which was originally
	 * separate from {@link SamReader} but in practice the two are always
	...
	class PrimitiveSamReaderToSamReaderAdapter implements SamReader, Indexing {
		...
		/**
		 * Wraps the boilerplate code for querying a record's mate, which is
		 * common across many implementations.
		 *
		 * @param rec
		 *            Record for which mate is sought. Must be a paired read.
		 * @return
		...
		public SAMRecordIterator query(final String sequence, final int start,
				final int end, final boolean contained) {
			return query(new QueryInterval[] { new QueryInterval(
					getFileHeader().getSequenceIndex(sequence), start, end) },
					contained);
		}
	...
	static class AssertingIterator implements SAMRecordIterator {
		...
		@Override
		public SAMRecordIterator assertSorted(
				...
				if (previous != null) {
					if (comparator.fileOrderCompare(previous, result) > 0) {
						throw new IllegalStateException(
								MessageFormat
										.format("Records {0} ({1}:{2}) should come after {3} ({4}:{5}) when sorting with {6}",
	...
	/**
	 * Internal interface for SAM/BAM/CRAM file reader implementations, as
	 * distinct from non-file-based readers.
	 *
	 ...
	 * TODO -- and either no-op or throw an exception for the others. TODO -- We
	 * should consider refactoring things to avoid this; TODO -- perhaps we can
	 * get away with not having this class at all.
	 */
	abstract class ReaderImplementation implements PrimitiveSamReader {
		abstract void enableFileSource(final SamReader reader,
