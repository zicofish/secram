 ...
 * Defaults are encoded in the class and are also overridable using system
 * properties.
 *
 * @author Tim Fennell
 */
public class Defaults {
	/**
	 * Should BAM index files be created when writing out coordinate sorted BAM
	 * files? Default = false.
	 */
	public static final boolean CREATE_INDEX;

	/**
	 * Should MD5 files be created when writing out SAM and BAM files? Default =
	 * false.
	 */
	public static final boolean CREATE_MD5;

	/**
	 * Should asynchronous I/O be used when writing out SAM and BAM files (one
	 * thread per file). Default = false.
	 */
	public static final boolean USE_ASYNC_IO;

	/**
	 * Compresion level to be used for writing BAM and other block-compressed
	 * outputs. Default = 5.
	 */
	...
	/**
	 * Even if BUFFER_SIZE is 0, this is guaranteed to be non-zero. If
	 * BUFFER_SIZE is non-zero, this == BUFFER_SIZE
	 */
	 ...
	 * for in the directory where the executable jar lives.
	 */
	public static final String INTEL_DEFLATER_SHARED_LIBRARY_PATH;

	/**
	 * The reference FASTA file. If this is not set, the file is null. This file
	 * may be required for reading writing SAM files (ex. CRAM).
	 */
	public static final File REFERENCE_FASTA;

	/**
	 * Custom reader factory able to handle URL based resources like ga4gh.
	 * Expected format: <url prefix>,<fully qualified factory class name>[,<jar
	 * file name>] E.g.
	 * https://www.googleapis.com/genomics/v1beta/reads/,com.google
	 * .genomics.ReaderFactory OR
	 * https://www.googleapis.com/genomics/v1beta/reads
	 * /,com.google.genomics.ReaderFactory,/tmp/genomics.jar
	 */
	...
	static {
		...
		TRY_USE_INTEL_DEFLATER = getBooleanProperty("try_use_intel_deflater",
				true);
		INTEL_DEFLATER_SHARED_LIBRARY_PATH = getStringProperty(
				"intel_deflater_so_path", null);
		if (BUFFER_SIZE == 0) {
	...
	/**
	 * Gets a boolean system property, prefixed with "samjdk." using the default
	 * if the property does not exist.
	 */
	...
	/**
	 * Gets an int system property, prefixed with "samjdk." using the default if
	...
	/**
	 * Gets a File system property, prefixed with "samdjk." using the default if
	 * the property does not exist.
	 */
