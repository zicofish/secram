...
public class CRAMIterator implements SAMRecordIterator {
	...
	private Iterator<SAMRecord> iterator = Collections.<SAMRecord> emptyList()
			.iterator();
	...
	public CRAMIterator(final InputStream inputStream,
		...
		final CramContainerIterator containerIterator = new CramContainerIterator(
	...
	public CRAMIterator(final SeekableStream seekableStream,
			final ReferenceSource referenceSource, final long[] coordinates)
			throws IOException {
		this.countingInputStream = new CountingInputStream(seekableStream);
		this.referenceSource = referenceSource;
		final CramSpanContainerIterator containerIterator = CramSpanContainerIterator
				.fromFileSpan(seekableStream, coordinates);
		cramHeader = containerIterator.getCramHeader();
		this.containerIterator = containerIterator;
	...
	private void nextContainer() throws IOException, IllegalArgumentException,
		...
		if (records == null)
			...
			records.clear();
		if (cramRecords == null)
			cramRecords = new ArrayList<CramCompressionRecord>(
			...
			if (validationStringency != ValidationStringency.SILENT
				...
				log.error(String
						.format("Reference sequence MD5 mismatch for slice: seq id %d, start %d, span %d, expected MD5 %s",
								...
								slice.alignmentSpan, String.format("%032x",
										new BigInteger(1, slice.refMD5))));
			}
		}
		...
		for (final CramCompressionRecord cramRecord : cramRecords) {
			final SAMRecord samRecord = cramToSamRecordFactory
					.create(cramRecord);
			if (!cramRecord.isSegmentUnmapped()) {
				final SAMSequenceRecord sequence = cramHeader
						.getSamFileHeader().getSequence(cramRecord.sequenceId);
				refs = referenceSource.getReferenceBases(sequence, true);
				if (samRecord.getReadBases() != SAMRecord.NULL_SEQUENCE)
					SequenceUtil.calculateMdAndNmTags(samRecord, refs,
							restoreMDTag, restoreNMTag);
			}
			...
			if (mReader != null) {
				final long chunkStart = (container.offset << 16)
						| cramRecord.sliceIndex;
	...
	public void jumpWithinContainerToPos(final int refIndex, final int pos) {
		...
		for (final SAMRecord record : records) {
			if (refIndex != SAMRecord.NO_ALIGNMENT_REFERENCE_INDEX
					&& record.getReferenceIndex() != refIndex)
				continue;
