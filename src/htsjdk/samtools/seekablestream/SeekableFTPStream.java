...
/**
 * Unfortunately the seekable stream classes exist for both Tribble and Picard,
 * and we need both. This class is for use with Tribble and delegates all the
 * work to a helper.
 *
 * @author jrobinso
 * @date Oct 27, 2010
...
public class SeekableFTPStream extends SeekableStream {
	...
	public SeekableFTPStream(URL url, UserPasswordInput userPasswordInput)
			throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		// "ftp://apache.cs.utah.edu/apache.org/HEADER.html" : args[0];
		String testURL = (args.length < 1) ? "ftp://hgdownload.cse.ucsc.edu/goldenPath/panTro3/vsHg19/panTro3.hg19.all.chain.gz"
				: args[0];
		long startPosition = (args.length < 2) ? 0x0b66c78l : Long
				.parseLong(args[1]);
		int len = (args.length < 3) ? 8 : Integer.parseInt(args[2]);
		int skipLen = (args.length < 4) ? 0x18 : Integer.parseInt(args[3]);
		SeekableStream s = SeekableStreamFactory.getInstance().getStreamFor(
				testURL);
		...
		if (args.length == 0) {
			...
			System.out.println("1) values do"
					+ (EXPECTED1.equals(data) ? "" : " not") + " match");
		}
		s.skip(skipLen);
		s.read(buffer, 0, len);
		if (s.position() != startPosition + 2 * len + skipLen
				&& s.position() != s.length()) {
		...
		if (args.length == 0) {
			...
			System.out.println("2) values do"
					+ (EXPECTED2.equals(data2) ? "" : " not") + " match");
		}
	}
}
