...
/**
 * This is a copy of java.util.zip.Deflater from OpenJDK 7, with the following
 * changes: - package and class name changed - static block to load
 * libIntelDeflater library - extends java.util.zip.Deflater so that
 * IntelDeflater object can be used as regular Deflater object. Note however
 * that all methods of Deflater are overridden.
 *
 * The shared library is found via one of the following mechanisms: 1. if
 * samjdk.intel_deflater_so_path system property is set, this is assumed to be
 * the path of libIntelDeflater.so 2. If system property is not set, directory
 * where the jarfile that this class came from lives is tried as location of
 * libIntelDeflater.so 3. If either of the above fails to find the library,
 * regular LD_LIBRARY_PATH is used to find the library. 4. If that doesn't work,
 * class fails to load and code falls back to regular Java Deflater class.
 *
 *
 ...
 * patents. It is fully described in the specifications at the <a
 * href="package-summary.html#package_description">java.util.zip package
 * description</a>.
 *
 * <p>
 * The following code fragment demonstrates a trivial compression and
 * decompression of a string using <tt>IntelDeflater</tt> and <tt>Inflater</tt>.
 *
 * <blockquote>
 * 
 * <pre>
...
public class IntelDeflater extends Deflater {
	...
	static {
		try {
			...
			if (Defaults.INTEL_DEFLATER_SHARED_LIBRARY_PATH != null) {
				...
				sharedLibrary = new File(
						Defaults.INTEL_DEFLATER_SHARED_LIBRARY_PATH);
			} else {
				// Look in directory containing this class for the library
				URL jarUrl = IntelDeflater.class.getProtectionDomain()
						.getCodeSource().getLocation();
				sharedLibrary = new File(
						new File(jarUrl.getPath()).getParentFile(),
						"libIntelDeflater.so");
			}
	 ...
	 * is called, the behavior of the IntelDeflater object is undefined.
	 */
	@Override
	public void end() {
		synchronized (zsRef) {
