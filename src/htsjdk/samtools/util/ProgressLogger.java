...
/**
 * Little progress logging class to facilitate consistent output of useful
 * information when progressing through a stream of SAM records.
 *
 * @author Tim Fennell
 */
public class ProgressLogger implements ProgressLoggerInterface {
	...
	/**
	 * Construct a progress logger with the desired log and frequency and the
	 * verb "Processed".
	 * 
	 * @param log
	 *            the Log object to write outputs to
	...
	/**
	 * Construct a progress logger with the desired log, the verb "Processed"
	 * and a period of 1m records.
	 * 
	 * @param log
	 *            the Log object to write outputs to
	 */
	...
	public synchronized boolean record(final String chrom, final int pos) {
		...
		if (++this.processed % this.n == 0) {
			...
			if (chrom == null)
				...
				readInfo = chrom + ":" + fmt.format(pos);

			log.info(this.verb, " ", processed, " " + noun
					+ ".  Elapsed time: ", elapsed, "s.  Time for last ",
					fmt.format(this.n), ": ", period,
					"s.  Last read position: ", readInfo);
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Records that a given record has been processed and triggers logging if
	 * necessary.
	 * 
	...
	private String formatElapseTime(final long seconds) {
		...
		return timeFmt.format(h) + ":" + timeFmt.format(m) + ":"
				+ timeFmt.format(s);
	}
}
