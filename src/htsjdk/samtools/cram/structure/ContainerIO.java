...
public class ContainerIO {
	...
	public static Container readContainer(final Version version,
		...
		if (container == null) {
			...
			return readContainer(version.major, new ByteArrayInputStream(
	 ...
	 * @throws IOException
	 */
	private static Container readContainer(final int major,
			final InputStream inputStream) throws IOException {
	...
	public static Container readContainerHeader(final int major,
		...
		if (!containerHeaderIO.readContainerHeader(major, container,
				inputStream)) {
			containerHeaderIO.readContainerHeader(container,
					new ByteArrayInputStream(
							(major >= 3 ? CramIO.ZERO_F_EOF_MARKER
									: CramIO.ZERO_B_EOF_MARKER)));
			return container;
		}
		return container;
	...
	private static Container readContainer(final int major,
			final InputStream inputStream, final int fromSlice,
	 ...
	 *             as per java IO contract
	 */
	public static int writeContainerHeader(final int major,
			final Container container, final OutputStream outputStream)
			throws IOException {
		return new ContainerHeaderIO().writeContainerHeader(major, container,
				outputStream);
	}
	 ...
	 *             as per java IO contract
	 */
	public static int writeContainer(final Version version,
			final Container container, final OutputStream outputStream)
			throws IOException {
		{
			if (container.blocks != null && container.blocks.length > 0) {
				...
				final boolean isFileHeaderContainer = firstBlock
						.getContentType() == BlockContentType.FILE_HEADER;
				if (isFileHeaderContainer) {
					...
					final int containerHeaderByteSize = new ContainerHeaderIO()
							.writeContainerHeader(version.major, container,
									outputStream);
					outputStream.write(byteArrayOutputStream.getBuffer(), 0,
							byteArrayOutputStream.size());
					return containerHeaderByteSize
							+ byteArrayOutputStream.size();
				}
			}
		}
		...
		int length = new ContainerHeaderIO().writeContainerHeader(
				version.major, container, outputStream);
		outputStream.write(byteArrayOutputStream.getBuffer(), 0,
				byteArrayOutputStream.size());
	 ...
	 *         out
	 */
	public static long getByteSize(final Version version,
