...
class SliceIO {
	...
	private static void readSliceHeadBlock(final int major, final Slice slice,
			final InputStream inputStream) throws IOException {
		slice.headerBlock = Block.readFromInputStream(major, inputStream);
		parseSliceHeaderBlock(major, slice);
	}

	private static void parseSliceHeaderBlock(final int major, final Slice slice)
			throws IOException {
		final InputStream inputStream = new ByteArrayInputStream(
				slice.headerBlock.getRawContent());
		...
		InputStreamUtils.readFully(inputStream, slice.refMD5, 0,
				slice.refMD5.length);

		final byte[] bytes = InputStreamUtils.readFully(inputStream);

		if (major >= CramVersions.CRAM_v3.major) {
			slice.sliceTags = BinaryTagCodec.readTags(bytes, 0, bytes.length,
	...
	private static byte[] createSliceHeaderBlockContent(final int major,
		...
		ITF8.writeUnsignedITF8(slice.embeddedRefBlockContentID,
				byteArrayOutputStream);
		byteArrayOutputStream.write(slice.refMD5 == null ? new byte[16]
				: slice.refMD5);

		if (major >= CramVersions.CRAM_v3.major) {
			if (slice.sliceTags != null) {
				final BinaryCodec binaryCoded = new BinaryCodec(
						byteArrayOutputStream);
				final BinaryTagCodec binaryTagCodec = new BinaryTagCodec(
						binaryCoded);
				SAMBinaryTagAndValue samBinaryTagAndValue = slice.sliceTags;
				do {
					log.debug("Writing slice tag: "
							+ SAMTagUtil.getSingleton().makeStringTag(
									samBinaryTagAndValue.tag));
					binaryTagCodec.writeTag(samBinaryTagAndValue.tag,
				...
				// In any case, not calling baseCodec.close() because it's
				// behaviour is
				// irrelevant here.
			}
		}
	...
	private static void readSliceBlocks(final int major, final Slice slice,
			final InputStream inputStream) throws IOException {
	...
	public static void write(final int major, final Slice slice,
			final OutputStream outputStream) throws IOException {

		slice.nofBlocks = 1 + slice.external.size()
				+ (slice.embeddedRefBlock == null ? 0 : 1);
		...
		slice.headerBlock = Block
				.buildNewSliceHeaderBlock(createSliceHeaderBlockContent(major,
						slice));
	...
	public static void read(final int major, final Slice slice,
			final InputStream inputStream) throws IOException {
