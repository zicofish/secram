...
public class Slice {
	...
	private void alignmentBordersSanityCheck(final byte[] ref) {
		...
		if (alignmentStart > ref.length) {
			log.error(String
					.format("Slice mapped outside of reference: seqID=%d, start=%d, counter=%d.",
		...
		if (alignmentStart - 1 + alignmentSpan > ref.length) {
			log.warn(String
					.format("Slice partially mapped outside of reference: seqID=%d, start=%d, span=%d, counter=%d.",
	...
	public boolean validateRefMD5(final byte[] ref) {
		...
		if (!validateRefMD5(ref, alignmentStart, alignmentSpan, refMD5)) {
			...
			final String excerpt = getBrief(alignmentStart, alignmentSpan, ref,
					shoulderLength);
	...
	private static boolean validateRefMD5(final byte[] ref,
			final int alignmentStart, final int alignmentSpan,
			final byte[] expectedMD5) {
		final int span = Math.min(alignmentSpan, ref.length - alignmentStart
				+ 1);
		final String md5 = SequenceUtil.calculateMD5String(ref,
				alignmentStart - 1, span);
		return md5.equals(String
				.format("%032x", new BigInteger(1, expectedMD5)));
	}

	private static String getBrief(final int startOneBased, final int span,
			final byte[] bases, final int shoulderLength) {
		...
		if (toExc - fromInc <= 2 * shoulderLength) {
			...
			sb.append(new String(Arrays.copyOfRange(bases, fromInc, fromInc
					+ shoulderLength)));
			sb.append("...");
			sb.append(new String(Arrays.copyOfRange(bases, toExc
					- shoulderLength, toExc)));
		}
	...
	public String toString() {
		return String.format("slice: seqID %d, start %d, span %d, records %d.",
	...
	public void setRefMD5(final byte[] ref) {
		...
		if (sequenceId < 0 && alignmentStart < 1) {
			...
			final int span = Math.min(alignmentSpan, ref.length
					- alignmentStart + 1);
	 ...
	 * 
	 * @param tag
	 *            tag ID as a short integer as returned by
	 *            {@link htsjdk.samtools.SAMTagUtil#makeBinaryTag(java.lang.String)}
