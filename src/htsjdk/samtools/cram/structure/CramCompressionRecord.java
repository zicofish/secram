...
public class CramCompressionRecord {
	...
	public boolean equals(final Object obj) {
		...
		if (alignmentStart != cramRecord.alignmentStart)
			return false;
		if (isNegativeStrand() != cramRecord.isNegativeStrand())
			return false;
		if (isVendorFiltered() != cramRecord.isVendorFiltered())
			return false;
		if (isSegmentUnmapped() != cramRecord.isSegmentUnmapped())
			return false;
		if (readLength != cramRecord.readLength)
			return false;
		if (isLastSegment() != cramRecord.isLastSegment())
			return false;
		if (recordsToNextFragment != cramRecord.recordsToNextFragment)
			return false;
		if (isFirstSegment() != cramRecord.isFirstSegment())
			return false;
		if (mappingQuality != cramRecord.mappingQuality)
			return false;

		if (!deepEquals(readFeatures, cramRecord.readFeatures))
			return false;

		if (!Arrays.equals(readBases, cramRecord.readBases))
			return false;
	...
	public String toString() {
		...
		if (readName != null)
			stringBuilder.append(readName).append("; ");
		...
		if (readBases != null)
			stringBuilder.append("; ").append("bases: ")
					.append(new String(readBases));
	...
	public void setMultiFragment(final boolean multiFragment) {
		flags = multiFragment ? flags | MULTI_FRAGMENT_FLAG : flags
				& ~MULTI_FRAGMENT_FLAG;
	}
	...
	public void setMateUnmapped(final boolean mateUnmapped) {
		mateFlags = mateUnmapped ? mateFlags | MATE_UNMAPPED_FLAG : mateFlags
				& ~MATE_UNMAPPED_FLAG;
	}
	...
	public void setForcePreserveQualityScores(
			final boolean forcePreserveQualityScores) {
