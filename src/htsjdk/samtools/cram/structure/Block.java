...
/**
 * Class representing CRAM block concept and some methods to operate with block
 * content. CRAM block is used to hold some (usually homogeneous) binary data.
 * An external compression can be applied to the content of a block. The class
 * provides some instantiation static methods, for example to read a block from
 * an input stream. Blocks can be written out to an output stream, this may be
...
public class Block {
	 ...
	 * @return a new {@link Block} object with fields and content from the input
	 *         stream
	 * @throws IOException
	 *             as per java IO contract
	 */
	public static Block readFromInputStream(final int major,
		...
		if (v3OrHigher) {
			final int actualChecksum = ((CRC32InputStream) inputStream)
					.getCRC32();
	...
	/**
	 * Create a new slice header block with the given uncompressed content. The
	 * block wil have RAW (no compression) and MAPPED_SLICE content type.
	 *
	 * @param rawContent
	...
	/**
	 * Write the block out to the the specified {@link OutputStream}. The method
	 * is parametrized with CRAM major version number.
	 *
	 * @param major
	 *            CRAM version major number
	 * @param outputStream
