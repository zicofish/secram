...
public class ExternalCompression {
	private static final int GZIP_COMPRESSION_LEVEL = Integer.valueOf(System
			.getProperty("gzip.compression.level", "5"));

	/**
	 * Compress a byte array into GZIP blob. The method obeys
	 * {@link ExternalCompression#GZIP_COMPRESSION_LEVEL} compression level.
	 *
	 * @param data
	...
	public static byte[] bzip2(final byte[] data) throws IOException {
		final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream(
				data.length * 2);
		final BZip2CompressorOutputStream bzip2CompressorOutputStream = new BZip2CompressorOutputStream(
				byteArrayOutputStream);
		bzip2CompressorOutputStream.write(data);
		bzip2CompressorOutputStream.close();
		return byteArrayOutputStream.toByteArray();
		// return InputStreamUtils.readFully(new BZip2CompressorInputStream(new
	...
	public static byte[] unbzip2(final byte[] data) throws IOException {
		final BZip2CompressorInputStream bzip2CompressorInputStream = new BZip2CompressorInputStream(
				new ByteArrayInputStream(data));
		return InputStreamUtils.readFully(bzip2CompressorInputStream);
		// final ByteArrayInputStream byteArrayInputStream = new
		// ByteArrayInputStream(data);
		// // hello, apache!
		// byteArrayInputStream.read();
		// byteArrayInputStream.read();
		// return InputStreamUtils.readFully(new
		// CBZip2InputStream(byteArrayInputStream));
	}
	...
	public static byte[] rans(final byte[] data, final int order) {
		final ByteBuffer buffer = RANS.compress(ByteBuffer.wrap(data),
				RANS.ORDER.fromInt(order), null);
		return toByteArray(buffer);
	}
	...
	public static byte[] xz(final byte[] data) throws IOException {
		final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream(
				data.length * 2);
		final XZCompressorOutputStream xzCompressorOutputStream = new XZCompressorOutputStream(
				byteArrayOutputStream);
	...
	/**
	 * Compress a byte array using Deflate
	 */
	public static byte[] deflate(final byte[] data) throws IOException {
		final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream(
				data.length * 2);
		final DeflateCompressorOutputStream deflateCompressorOutputStream = new DeflateCompressorOutputStream(
				byteArrayOutputStream);
		deflateCompressorOutputStream.write(data);
		deflateCompressorOutputStream.close();
		return byteArrayOutputStream.toByteArray();
	}

	/**
	 * Uncompress a Deflated blob into a new byte array
	 */
	public static byte[] inflate(final byte[] data) throws IOException {
		final DeflateCompressorInputStream deflateCompressorInputStream = new DeflateCompressorInputStream(
				new ByteArrayInputStream(data));
		return InputStreamUtils.readFully(deflateCompressorInputStream);
	}
