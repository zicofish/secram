	...
	public void write(final CramCompressionRecord r) throws IOException {
		...
		if (r.tags != null) {
			for (int i = 0; i < r.tags.length; i++) {
				final DataWriter<byte[]> writer = tagValueCodecs
		...
		if (!r.isSegmentUnmapped()) {
			...
			for (final ReadFeature f : r.readFeatures) {
				...
				case Substitution.operator:
					...
					if (sv.getCode() < 0)
						baseSubstitutionCodeCodec.writeData(substitutionMatrix
								.code(sv.getReferenceBase(), sv.getBase()));
					else
						baseSubstitutionCodeCodec.writeData(sv.getCode());
					// baseSubstitutionCodec.writeData((byte)
					// sv.getBaseChange().getChange());
					break;
