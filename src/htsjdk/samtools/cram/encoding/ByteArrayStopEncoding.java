...
public class ByteArrayStopEncoding implements Encoding<byte[]> {
	...
	public static EncodingParams toParam(final byte stopByte,
	...
	public BitCodec<byte[]> buildCodec(
		...
		final InputStream is = inputMap == null ? null : inputMap
				.get(externalId);
		final ExposedByteArrayOutputStream os = outputMap == null ? null
				: outputMap.get(externalId);
	...
	public static class ByteArrayStopCodec extends AbstractBitCodec<byte[]> {
		...
		public ByteArrayStopCodec(final byte stopByte,
		...
		@Override
		public byte[] read(final BitInputStream bitInputStream)
				throws IOException {
			readingBAOS.reset();
		...
		public long write(final BitOutputStream bitOutputStream,
				final byte[] object) throws IOException {
