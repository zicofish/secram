...
public class DataReaderFactory {
	...
	public AbstractReader buildReader(final AbstractReader reader,
			...
			final CompressionHeader header, final int refId)
		...
		for (final Field field : reader.getClass().getFields()) {
			if (field.isAnnotationPresent(DataSeries.class)) {
				final DataSeries dataSeries = field
						.getAnnotation(DataSeries.class);
				...
				field.set(
						reader,
						createReader(type, header.encodingMap.get(key),
								bitInputStream, inputMap));
			}

			if (field.isAnnotationPresent(DataSeriesMap.class)) {
				final DataSeriesMap dataSeriesMap = field
						.getAnnotation(DataSeriesMap.class);
	...
	private <T> DataReader<T> createReader(final DataSeriesType valueType,
		...
		final Encoding<T> encoding = encodingFactory.createEncoding(valueType,
				params.id);
		if (encoding == null)
	...
	private static <T> DataReader<T> buildNullReader(
	...
	private static class DefaultDataReader<T> implements DataReader<T> {
		...
		public DefaultDataReader(final BitCodec<T> codec,
				final BitInputStream bitInputStream) {
			this.codec = codec;
	...
	public Map<String, DataReaderWithStats> getStats(
			final CramRecordReader reader) throws IllegalArgumentException,
