...
class Decoding {
	...
	static void RansDecSymbolInit(final RansDecSymbol symbol, final int start,
			final int freq) {
		assert (start <= (1 << 16));
	...
	private static int RansDecAdvanceStep(final int r, final int start,
			final int freq, final int scale_bits) {
	...
	static int RansDecAdvanceSymbolStep(final int r, final RansDecSymbol sym,
	...
	static int RansDecAdvanceSymbol(final int r, final ByteBuffer pptr,
			final RansDecSymbol sym, final int scale_bits) {
		return Decoding
	...
	private static int RansDecAdvance(int r, final ByteBuffer pptr,
			final int start, final int freq, final int scale_bits) {
		final int mask = (1 << scale_bits) - 1;
