...
class Frequencies {
	...
	static void readStats_o0(final ByteBuffer cp,
			final Decoding.AriDecoder decoder,
		...
		do {
			...
			if ((decoder.fc[j].F = (cp.get() & 0xFF)) >= 128) {
				decoder.fc[j].F &= ~128;
				decoder.fc[j].F = ((decoder.fc[j].F & 127) << 8)
						| (cp.get() & 0xFF);
			}
			decoder.fc[j].C = x;

			Decoding.RansDecSymbolInit(syms[j], decoder.fc[j].C,
					decoder.fc[j].F);
	...
	static void readStats_o1(final ByteBuffer cp,
			final Decoding.AriDecoder[] D, final Decoding.RansDecSymbol[][] syms) {
		int rle_i = 0;
		int i = 0xFF & cp.get();
		do {
			int rle_j = 0;
