...
public class ReferenceSource {
	...
	byte[] findBasesByName(final String name, final boolean tryVariants) {
		...
		try {
			...
			// the only way to test if rsFile contains the sequence is to try
			// and catch exception.
		}
		if (sequence != null)
	...
	byte[] findBasesByMD5(final String md5) throws IOException {
		final String url = String.format(
				Defaults.EBI_REFERENCE_SEVICE_URL_MASK, md5);
		...
		for (int i = 0; i < downloadTriesBeforeFailing; i++) {
			...
			try {
				final String downloadedMD5 = SequenceUtil
						.calculateMD5String(data);
	...
	public void setDownloadTriesBeforeFailing(
			final int downloadTriesBeforeFailing) {
