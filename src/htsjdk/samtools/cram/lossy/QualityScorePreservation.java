...
public class QualityScorePreservation {
	...
	private static List<PreservationPolicy> parsePolicies(final String spec) {
		...
		Collections.sort(policyList, new Comparator<PreservationPolicy>() {
			...
			@Override
			public int compare(final PreservationPolicy o1,
					final PreservationPolicy o2) {
	...
	private static PreservationPolicy parseSinglePolicy(final String spec) {
		...
		while (!list.isEmpty()) {
			...
			case 'X':
				...
				preservationPolicy.baseCategories.add(BaseCategory
						.lowerThanCoverage(coverage));
				break;
			case 'D':
				preservationPolicy.baseCategories.add(BaseCategory
						.flankingDeletion());
			...
			case 'M':
				...
				preservationPolicy.readCategory = ReadCategory
						.higher_than_mapping_score(score);
				break;
			case 'm':
				score = readParam(list);
				preservationPolicy.readCategory = ReadCategory
						.lower_than_mapping_score(score);
				break;
			case 'U':
	...
	private static byte applyTreatment(final byte score,
			final QualityScoreTreatment qualityScoreTreatment) {
	...
	public void addQualityScores(final SAMRecord samRecord,
		...
		for (final PreservationPolicy preservationPolicy : policyList)
			addQS(samRecord, cramRecord, scores, referenceTracks,
					preservationPolicy);
		...
		if (!cramRecord.isForcePreserveQualityScores()) {
			for (int i = 0; i < scores.length; i++) {
				if (scores[i] > -1) {
					...
					cramRecord.readFeatures.add(new BaseQualityScore(i + 1,
							scores[i]));
				}
			}
			if (cramRecord.readFeatures != null)
				Collections.sort(cramRecord.readFeatures,
	...
	public boolean areReferenceTracksRequired() {
		if (policyList == null || policyList.isEmpty())
			return false;
		for (final PreservationPolicy preservationPolicy : policyList) {
			if (preservationPolicy.baseCategories == null
					|| preservationPolicy.baseCategories.isEmpty())
				continue;
	...
	private static void addQS(final SAMRecord samRecord,
			final CramCompressionRecord cramRecord, final byte[] scores,
			final ReferenceTracks referenceTracks,
			final PreservationPolicy preservationPolicy) {
		final int alSpan = samRecord.getAlignmentEnd()
		...
		for (final BaseCategory baseCategory : preservationPolicy.baseCategories) {
			...
			case FLANKING_DELETION:
				pos = 0;
				for (final CigarElement cigarElement : samRecord.getCigar()
						.getCigarElements()) {
				...
				for (final CigarElement cigarElement : samRecord.getCigar()
					...
					if (cigarElement.getOperator().consumesReadBases()) {
						for (int i = 0; i < cigarElement.getLength(); i++) {
							final boolean match = samRecord.getReadBases()[pos
									+ i] == referenceTracks.baseAt(refPos + i);
					...
					refPos += cigarElement.getOperator()
							.consumesReferenceBases() ? cigarElement
							.getLength() : 0;
				}
				break;
			case INSERTION:
			...
			case LOWER_COVERAGE:
				...
				for (final CigarElement cigarElement : samRecord.getCigar()
					...
					case X:
						for (int i = 0; i < cigarElement.getLength(); i++)
							mask[pos + i - 1] = referenceTracks
									.coverageAt(refPos + i) < baseCategory.param;
						break;
					default:
						break;
					}
		...
		for (int i = 0; i < mask.length; i++)
			if (mask[i]) {
				scores[i] = applyTreatment(qualityScores[i],
						preservationPolicy.treatment);
				maskedCount++;
			}
