...
public class CompressionHeaderFactory {
	...
	private static final Log log = Log
			.getInstance(CompressionHeaderFactory.class);
	...
	public CompressionHeader build(final List<CramCompressionRecord> records,
		...
		{ // bit flags encoding:
			getOptimalIntegerEncoding(header, EncodingKey.BF_BitFlags, 0,
					records);
		}

		{ // compression bit flags encoding:
			getOptimalIntegerEncoding(header,
					EncodingKey.CF_CompressionBitFlags, 0, records);
		}
		...
		{ // read length encoding:
			getOptimalIntegerEncoding(header, EncodingKey.RL_ReadLength, 0,
					records);
		}
		...
		{ // alignment offset:
			if (sorted) { // alignment offset:
				...
				header.externalCompressors.put(aStartID,
						ExternalCompressor.createRANS(RANS.ORDER.ONE));
				log.debug("Assigned external id to alignment starts: "
						+ aStartID);
			}
		}

		{ // read group
			getOptimalIntegerEncoding(header, EncodingKey.RG_ReadGroup, -1,
					records);
		}
		...
		{ // read name encoding:
			...
			header.encodingMap.put(EncodingKey.RN_ReadName,
		...
		{ // tag count
			...
			header.encodingMap.put(EncodingKey.TC_TagCount,
					HuffmanIntegerEncoding.toParam(calculator.values(),
							calculator.bitLens()));
		}
		...
		{ // tag name and type
			...
			header.encodingMap.put(EncodingKey.TN_TagNameAndType,
					HuffmanIntegerEncoding.toParam(calculator.values(),
							calculator.bitLens()));
		}

		{
			...
			final Map<byte[], MutableInt> map = new TreeMap<byte[], MutableInt>(
					baComparator);
			...
			for (final CramCompressionRecord record : records) {
				...
				for (int i = 0; i < record.tags.length; i++) {
					record.tagIds[i * 3] = (byte) record.tags[tagIndex].keyType3Bytes
							.charAt(0);
					record.tagIds[i * 3 + 1] = (byte) record.tags[tagIndex].keyType3Bytes
							.charAt(1);
					record.tagIds[i * 3 + 2] = (byte) record.tags[tagIndex].keyType3Bytes
							.charAt(2);
					tagIndex++;
				}
		...
		{ // feature position
			...
			final Encoding<Integer> bestEncoding = calc.getBestEncoding();
			header.encodingMap.put(
					EncodingKey.FP_FeaturePosition,
					new EncodingParams(bestEncoding.id(), bestEncoding
							.toByteArray()));
		}
		...
		{ // feature code
			...
			header.encodingMap.put(EncodingKey.FC_FeatureCode,
					HuffmanByteEncoding.toParam(calculator.valuesAsBytes(),
							calculator.bitLens));
		}

		{ // bases:
			header.encodingMap.put(EncodingKey.BA_Base,
					ExternalByteEncoding.toParam(baseID));
		}

		{ // quality scores:
			header.encodingMap.put(EncodingKey.QS_QualityScore,
					...
					if (record.readFeatures != null)
						for (final ReadFeature readFeature : record.readFeatures)
							if (readFeature.getOperator() == Substitution.operator) {
								...
								final byte refBase = substitution
										.getReferenceBase();
						...
						if (recordFeature.getOperator() == Substitution.operator) {
							...
							if (substitution.getCode() == -1) {
								final byte refBase = substitution
										.getReferenceBase();
								final byte base = substitution.getBase();
								substitution.setCode(header.substitutionMatrix
										.code(refBase, base));
							}
		...
		{ // insertion bases
			header.encodingMap.put(EncodingKey.IN_Insertion,
					ByteArrayStopEncoding.toParam((byte) 0, baseID));
		}

		{ // insertion bases
			header.encodingMap.put(EncodingKey.SC_SoftClip,
					ByteArrayStopEncoding.toParam((byte) 0, baseID));
		}
				...
				if (record.readFeatures != null)
					for (final ReadFeature recordFeature : record.readFeatures)
						if (recordFeature.getOperator() == Deletion.operator)
							calculator.add(((Deletion) recordFeature)
									.getLength());
			calculator.calculate();
				...
				if (record.readFeatures != null)
					for (final ReadFeature recordFeature : record.readFeatures)
						if (recordFeature.getOperator() == HardClip.operator)
							calculator.addValue(((HardClip) recordFeature)
									.getLength());

			final Encoding<Integer> bestEncoding = calculator.getBestEncoding();
			header.encodingMap.put(EncodingKey.HC_HardClip, new EncodingParams(
				...
				if (record.readFeatures != null)
					for (final ReadFeature recordFeature : record.readFeatures)
						if (recordFeature.getOperator() == Padding.operator)
							calculator.addValue(((Padding) recordFeature)
									.getLength());

			final Encoding<Integer> bestEncoding = calculator.getBestEncoding();
			header.encodingMap.put(EncodingKey.PD_padding, new EncodingParams(
					bestEncoding.id(), bestEncoding.toByteArray()));
				...
				if (record.readFeatures != null)
					for (final ReadFeature recordFeature : record.readFeatures)
						if (recordFeature.getOperator() == RefSkip.operator)
							calculator.add(((RefSkip) recordFeature)
									.getLength());
			calculator.calculate();

			header.encodingMap.put(EncodingKey.RS_RefSkip,
					HuffmanIntegerEncoding.toParam(calculator.values,
							calculator.bitLens));
		}
		...
		{ // next fragment ref id:
			...
			if (calculator.values.length == 0)
				header.encodingMap.put(
						EncodingKey.NS_NextFragmentReferenceSequenceID,
			...
			log.debug("NS: "
					+ header.encodingMap
							.get(EncodingKey.NS_NextFragmentReferenceSequenceID));
		}
		...
		{ // template size
			header.encodingMap.put(EncodingKey.TS_InsetSize,
					ExternalIntegerEncoding.toParam(mateInfoID));
		}
	...
	private static int getValue(final EncodingKey key,
	...
	private static void getOptimalIntegerEncoding(
			final CompressionHeader header, final EncodingKey key,
			final int minValue, final List<CramCompressionRecord> records) {
		final IntegerEncodingCalculator calc = new IntegerEncodingCalculator(
				key.name(), minValue);
		for (final CramCompressionRecord record : records) {
	...
	private static class BitCode implements Comparable<BitCode> {
		...
		public int compareTo(
				@SuppressWarnings("NullableProblems") final BitCode o) {
	...
	public static class IntegerEncodingCalculator {
		...
		public IntegerEncodingCalculator(final String name,
				final int dictionaryThreshold, final int minValue) {
			this.name = name;
			...
			calculators.add(new EncodingLengthCalculator(
					new GammaIntegerEncoding(1 - minValue)));

			for (int i = 2; i < 5; i++)
				calculators.add(new EncodingLengthCalculator(
						new SubexponentialIntegerEncoding(0 - minValue, i)));
		...
		public Encoding<Integer> getBestEncoding() {
			if (dictionary != null && dictionary.size() == 1) {
				...
				final EncodingParams param = HuffmanIntegerEncoding.toParam(
			...
			{ // check if beta is better:
				...
				final int betaLength = (int) Math.round(Math
						.log(max - minValue) / Math.log(2) + 0.5);
			...
			{ // try huffman:
				if (dictionary != null) {
					...
					for (final Integer value : dictionary.keySet())
						huffmanParamsCalculator.add(value,
								dictionary.get(value).value);

					huffmanParamsCalculator.calculate();

					final EncodingParams param = HuffmanIntegerEncoding
							.toParam(huffmanParamsCalculator.values(),
									huffmanParamsCalculator.bitLens());
					final HuffmanIntegerEncoding huffmanEncoding = new HuffmanIntegerEncoding();
					huffmanEncoding.fromByteArray(param.params);
					final EncodingLengthCalculator calculator = new EncodingLengthCalculator(
							huffmanEncoding);
