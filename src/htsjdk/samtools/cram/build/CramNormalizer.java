...
public class CramNormalizer {
	...
	public CramNormalizer(final SAMFileHeader header,
	...
	public void normalize(final ArrayList<CramCompressionRecord> records,
			...
			for (final CramCompressionRecord record : records) {
				...
				if (record.isHasMateDownStream()) {
					final CramCompressionRecord downMate = records
							.get(record.index + record.recordsToNextFragment
									- startCounter);
		...
		for (final CramCompressionRecord record : records) {
			...
			if (record.isUnknownBases()) {
				...
				record.readBases = restoreReadBases(record, refBases,
	...
	private static void setNextMate(final CramCompressionRecord record,
			final CramCompressionRecord next) {
			...
			if (record.readFeatures != null)
				for (final ReadFeature feature : record.readFeatures) {
					...
					case BaseQualityScore.operator:
						...
						scores[pos - 1] = ((BaseQualityScore) feature)
								.getQualityScore();
						star = false;
						break;
					case ReadBase.operator:
						pos = feature.getPosition();
						scores[pos - 1] = ((ReadBase) feature)
								.getQualityScore();
						star = false;
						break;
	...
	private static byte[] restoreReadBases(final CramCompressionRecord record,
			final byte[] ref, final int refOffsetZeroBased,
			final SubstitutionMatrix substitutionMatrix) {
		...
		for (final ReadFeature variation : variations) {
			...
			case Substitution.operator:
				...
				final byte base = substitutionMatrix.base(refBase,
						substitution.getCode());
	...
	public static int computeInsertSize(final CramCompressionRecord firstEnd,
		...
		final int firstEnd5PrimePosition = firstEnd.isNegativeStrand() ? firstEnd
				.getAlignmentEnd() : firstEnd.alignmentStart;
		final int secondEnd5PrimePosition = secondEnd.isNegativeStrand() ? secondEnd
				.getAlignmentEnd() : secondEnd.alignmentStart;
