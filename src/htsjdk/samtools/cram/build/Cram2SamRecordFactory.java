...
public class Cram2SamRecordFactory {
	...
	public SAMRecord create(final CramCompressionRecord cramRecord) {
		...
		if (cramRecord.readGroupID > -1) {
			final SAMReadGroupRecord readGroupRecord = header.getReadGroups()
					.get(cramRecord.readGroupID);
	...
	private static void copyFlags(final CramCompressionRecord cramRecord,
		...
		samRecord.setReadFailsVendorQualityCheckFlag(cramRecord
				.isVendorFiltered());
	...
	private static Cigar getCigar2(final Collection<ReadFeature> features,
		...
		if (features == null || features.isEmpty()) {
			final CigarElement cigarElement = new CigarElement(readLength,
					CigarOperator.M);
		...
		if (lastOperator != null) {
			if (lastOperator != CigarOperator.M) {
				...
				if (readLength >= lastOpPos + lastOpLen) {
					cigarElement = new CigarElement(readLength
							- (lastOpLen + lastOpPos) + 1, CigarOperator.M);
