 ...
 * {@link htsjdk.samtools.seekablestream.SeekableStream}. The locations are
 * specified with pairs of coordinates, they are basically file pointers as
 * returned for example by
 * {@link htsjdk.samtools.SamReader.Indexing#getFilePointerSpanningReads()}
 */
public class CramSpanContainerIterator implements Iterator<Container> {
	...
	private CramSpanContainerIterator(final SeekableStream seekableStream,
			final long[] coordinates) throws IOException {
	...
	private class Boundary {
		...
		public Boundary(final long start, final long end) {
			...
			if (start >= end)
				throw new RuntimeException(
						"Boundary start is greater than end.");
		}
		...
		Container next() throws IOException {
			...
			if (seekableStream.position() > (end >> 16))
				throw new RuntimeException(
						"No more containers in this boundary.");
			final long offset = seekableStream.position();
			final Container c = ContainerIO.readContainer(
					cramHeader.getVersion(), seekableStream);
			c.offset = offset;
			return c;
		}
	}
