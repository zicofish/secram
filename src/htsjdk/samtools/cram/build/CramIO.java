...
public class CramIO {
	/**
	 * The 'zero-B' EOF marker as per CRAM specs v2.1. This is basically a
	 * serialized empty CRAM container with sequence id set to some number to
	 * spell out 'EOF' in hex.
	 */
	public static final byte[] ZERO_B_EOF_MARKER = bytesFromHex("0b 00 00 00 ff ff ff ff ff e0 45 4f 46 00 00 00 00 01 00 00 01 00 06 06 01 00 "
			+ "" + "01 00 01 00");
	/**
	 * The zero-F EOF marker as per CRAM specs v3.0. This is basically a
	 * serialized empty CRAM container with sequence id set to some number to
	 * spell out 'EOF' in hex.
	 */
	...
	/**
	 * Write an end-of-file marker to the {@link OutputStream}. The specific EOF
	 * marker is chosen based on the CRAM version.
	 *
	 * @param version
	 ...
	 */
	public static long issueEOF(final Version version,
	...
	private static boolean streamEndsWith(final SeekableStream seekableStream,
			final byte[] marker) throws IOException {
	...
	/**
	 * Check if the {@link SeekableStream} is properly terminated with a
	 * end-of-file marker.
	 *
	 * @param version
	 ...
	 * @return true if the stream ends with a correct EOF marker, false
	 *         otherwise
	 * @throws IOException
	 *             as per java IO contract
	 */
	@SuppressWarnings("SimplifiableIfStatement")
	private static boolean checkEOF(final Version version,
			final SeekableStream seekableStream) throws IOException {
	...
	/**
	 * Check if the file: 1) contains proper CRAM header. 2) given the version
	 * info from the header check the end of file marker.
	 *
	 * @param file
	 *            the CRAM file to check
	 * @return true if the file is a valid CRAM file and is properly terminated
	 ...
	 *
	 * @param cramHeader
	 *            the {@link CramHeader} object to write
	 * @param outputStream
	 ...
	 *             as per java IO contract
	 */
	public static long writeCramHeader(final CramHeader cramHeader,
			final OutputStream outputStream) throws IOException {
		// if (cramHeader.getVersion().major < 3) throw new
		// RuntimeException("Deprecated CRAM version: " +
		...
		final long length = CramIO.writeContainerForSamFileHeader(
				cramHeader.getVersion().major, cramHeader.getSamFileHeader(),
				outputStream);

		return CramIO.DEFINITION_LENGTH + length;
	}

	private static CramHeader readFormatDefinition(final InputStream inputStream)
			throws IOException {
		...
		final Version version = new Version(inputStream.read(),
				inputStream.read(), 0);
	...
	public static CramHeader readCramHeader(final InputStream inputStream)
		...
		final SAMFileHeader samFileHeader = readSAMFileHeader(
				header.getVersion(), inputStream, new String(header.getId()));

		return new CramHeader(header.getVersion(), new String(header.getId()),
				samFileHeader);
	}
	...
	private static long writeContainerForSamFileHeader(final int major,
			final SAMFileHeader samFileHeader, final OutputStream os)
			throws IOException {
		...
		final int containerHeaderByteSize = ContainerIO.writeContainerHeader(
				major, container, os);
	...
	private static SAMFileHeader readSAMFileHeader(final Version version,
			InputStream inputStream, final String id) throws IOException {
		final Container container = ContainerIO.readContainerHeader(
				version.major, inputStream);
		final Block block;
		{
			if (version.compatibleWith(CramVersions.CRAM_v3)) {
				final byte[] bytes = new byte[container.containerByteSize];
				InputStreamUtils.readFully(inputStream, bytes, 0, bytes.length);
				block = Block.readFromInputStream(version.major,
						new ByteArrayInputStream(bytes));
				// ignore the rest of the container
			} else {
				/*
				 * pending issue: container.containerByteSize inputStream 2
				 * bytes shorter then needed in the v21 test cram files.
				 */
	 ...
	 * succeed only if there is sufficient space reserved in the existing CRAM
	 * header. The implementation re-writes the first FILE_HEADER block in the
	 * first container of the CRAM file using random file access.
	 *
	 * @param file
	 *            the CRAM file
	 * @param newHeader
	 *            the new CramHeader container a new SAM file header
	 * @return true if successfully replaced the header, false otherwise
	 * @throws IOException
	 *             as per java IO contract
	 */
	public static boolean replaceCramHeader(final File file,
			final CramHeader newHeader) throws IOException {

		final CountingInputStream countingInputStream = new CountingInputStream(
				new FileInputStream(file));

		final CramHeader header = readFormatDefinition(countingInputStream);
		final Container c = ContainerIO.readContainerHeader(
				header.getVersion().major, countingInputStream);
		final long pos = countingInputStream.getCount();
		countingInputStream.close();

		final Block block = Block.buildNewFileHeaderBlock(toByteArray(newHeader
				.getSamFileHeader()));
		final ExposedByteArrayOutputStream byteArrayOutputStream = new ExposedByteArrayOutputStream();
