...
/**
 * Describes a SAM-like resource, including its data (where the records are),
 * and optionally an index.
 * <p/>
 * A data or index source may originate from a {@link java.io.File},
 * {@link java.io.InputStream}, {@link URL}, or
 * {@link htsjdk.samtools.seekablestream.SeekableStream}; look for the
 * appropriate overload for {@code htsjdk.samtools.SamInputResource#of()}.
 *
 * @author mccowan
 */
public class SamInputResource {
	...
	/**
	 * Creates a {@link SamInputResource} reading from the provided resource,
	 * with no index.
	 */
	...
	public static SamInputResource of(final SeekableStream seekableStream) {
		return new SamInputResource(new SeekableStreamInputResource(
				seekableStream));
	}

	/**
	 * Creates a {@link SamInputResource} from a string specifying *either* a
	 * url or a file path
	 */
...
/**
 * Describes an arbitrary input source, which is something that can be accessed
 * as either a {@link htsjdk.samtools.seekablestream.SeekableStream} or
 * {@link java.io.InputStream}. A concrete implementation of this class exists
 * for each of {@link InputResource.Type}.
 */
abstract class InputResource {
	...
	/**
	 * Returns null if this resource cannot be represented as a
	 * {@link htsjdk.samtools.seekablestream.SeekableStream}.
	 */
...
class FileInputResource extends InputResource {
	...
	final Lazy<SeekableStream> lazySeekableStream = new Lazy<SeekableStream>(
			new Lazy.LazyInitializer<SeekableStream>() {
				@Override
