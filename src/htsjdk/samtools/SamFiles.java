...
public class SamFiles {
	/**
	 * Finds the index file associated with the provided SAM file. The index
	 * file must exist and be reachable to be found.
	 *
	...
	public static File findIndex(final File samFile) {
		...
		if (fileName.endsWith(BamFileIoUtils.BAM_FILE_EXTENSION)) {
			final String bai = fileName.substring(0, fileName.length()
					- BamFileIoUtils.BAM_FILE_EXTENSION.length())
		...
		indexFile = new File(samFile.getParent(), samFile.getName()
				+ BAMIndex.BAMIndexSuffix);
		if (indexFile.isFile()) {
			return indexFile;
		} else {
