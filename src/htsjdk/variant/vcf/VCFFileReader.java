...
public class VCFFileReader implements Closeable, Iterable<VariantContext> {
	...
	public static SAMSequenceDictionary getSequenceDictionary(final File file) {
		final SAMSequenceDictionary dict = new VCFFileReader(file, false)
	...
	/**
	 * Allows construction of a VCFFileReader that will or will not assert the
	 * presence of an index as desired.
	 */
	public VCFFileReader(final File file, final boolean requireIndex) {
		// Note how we deal with type safety here, just casting to
		// (FeatureCodec)
		// in the call to getFeatureReader is not enough for Java 8.
		FeatureCodec<VariantContext, ?> codec = isBCF(file) ? new BCF2Codec()
				: new VCFCodec();
		this.reader = AbstractFeatureReader.getFeatureReader(
				file.getAbsolutePath(), codec, requireIndex);
	}
	...
	public VCFFileReader(final File file, final File indexFile,
		...
		FeatureCodec<VariantContext, ?> codec = isBCF(file) ? new BCF2Codec()
				: new VCFCodec();
		this.reader = AbstractFeatureReader.getFeatureReader(
				file.getAbsolutePath(), indexFile.getAbsolutePath(), codec,
				requireIndex);
	}

	/**
	 * Parse a VCF file and convert to an IntervalList The name field of the
	 * IntervalList is taken from the ID field of the variant, if it exists. if
	 * not, creates a name of the format interval-n where n is a running number
	 * that increments only on un-named intervals
	 * 
	 * @param file
	 * @return
	 */
	...
	/**
	 * Converts a vcf to an IntervalList. The name field of the IntervalList is
	 * taken from the ID field of the variant, if it exists. If not, creates a
	 * name of the format interval-n where n is a running number that increments
	 * only on un-named intervals Will use a "END" tag in the info field as the
	 * end of the interval (if exists).
	 * 
	 * @param vcf
	 *            the vcfReader to be used for the conversion
	...
	public static IntervalList fromVcf(final VCFFileReader vcf,
		...
		final SAMSequenceDictionary dict = vcf.getFileHeader()
		...
		for (final VariantContext vc : vcf) {
			if (includeFiltered || !vc.isFiltered()) {
				...
				final Integer intervalEnd = vc.getCommonInfo()
						.getAttributeAsInt("END", vc.getEnd());
				if (".".equals(name) || name == null)
					name = "interval-" + (++intervals);
				list.add(new Interval(vc.getChr(), vc.getStart(), intervalEnd,
						false, name));
			}
		}
	...
	public CloseableIterator<VariantContext> iterator() {
		try {
			...
			throw new TribbleException(
					"Could not create an iterator from a feature reader.", ioe);
		}
	}
	...
	public void close() {
		try {
			...
			throw new TribbleException(
					"Could not close a variant context feature reader.", ioe);
		}
	}
}
