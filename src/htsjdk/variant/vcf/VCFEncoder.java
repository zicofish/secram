...
public class VCFEncoder {
	...
	/**
	 * Prepare a VCFEncoder that will encode records appropriate to the given
	 * VCF header, optionally allowing missing fields in the header.
	 */
	public VCFEncoder(final VCFHeader header,
	...
	public String encode(final VariantContext context) {
		...
		// FILTER
		stringBuilder.append(getFilterString(context)).append(
				VCFConstants.FIELD_SEPARATOR);

		// INFO
		final Map<String, String> infoFields = new TreeMap<String, String>();
		for (final Map.Entry<String, Object> field : context.getAttributes()
				.entrySet()) {
		...
		if (gc.isLazyWithData()
			...
			stringBuilder.append(VCFConstants.FIELD_SEPARATOR);
			stringBuilder.append(((LazyGenotypesContext) gc)
					.getUnparsedGenotypeData().toString());
		} else {
			final List<String> genotypeAttributeKeys = context
					.calcVCFGenotypeKeys(this.header);
			if (!genotypeAttributeKeys.isEmpty()) {
				...
				addGenotypeData(context, alleleStrings, genotypeAttributeKeys,
						stringBuilder);
			}
		}
	...
	private String formatQualValue(final double qual) {
		...
		if (s.endsWith(QUAL_FORMAT_EXTENSION_TO_TRIM))
			s = s.substring(0,
					s.length() - QUAL_FORMAT_EXTENSION_TO_TRIM.length());
		return s;
	}

	private void fieldIsMissingFromHeaderError(final VariantContext vc,
	 ...
	 *
	 * Large doubles =&gt; gets %.2f style formatting Doubles &lt; 1 / 10 but
	 * &gt; 1/100 =&gt; get %.3f style formatting Double &lt; 1/100 =&gt; %.3e
	 * formatting
	 * 
	 * @param d
	 * @return
	 ...
	 */
	public void addGenotypeData(final VariantContext vc,
			final Map<Allele, String> alleleMap,
			final List<String> genotypeFormatKeys, final StringBuilder builder) {
		...
		for (final String sample : this.header.getGenotypeSamples()) {
			...
			for (final String field : genotypeFormatKeys) {
				if (field.equals(VCFConstants.GENOTYPE_KEY)) {
					...
					for (int i = 1; i < g.getPloidy(); i++) {
						builder.append(g.isPhased() ? VCFConstants.PHASED
					...
					if (field.equals(VCFConstants.GENOTYPE_FILTER_KEY)) {
						...
						final IntGenotypeFieldAccessors.Accessor accessor = GENOTYPE_FIELD_ACCESSORS
								.getAccessor(field);
						if (accessor != null) {
							...
							final VCFFormatHeaderLine metaData = this.header
									.getFormatHeaderLine(field);
							if (metaData != null) {
								final int numInFormatField = metaData
										.getCount(vc);
			...
			for (int i = 0; i < attrs.size(); i++) {
				if (i > 0
						|| genotypeFormatKeys
								.contains(VCFConstants.GENOTYPE_KEY)) {
	 ...
	 */
	private void writeInfoString(final Map<String, String> infoFields,
		...
		for (final Map.Entry<String, String> entry : infoFields.entrySet()) {
			...
			if (!entry.getValue().equals("")) {
				final VCFInfoHeaderLine metaData = this.header
						.getInfoHeaderLine(entry.getKey());
				if (metaData == null
	...
	private void writeAllele(final Allele allele,
			final Map<Allele, String> alleleMap, final StringBuilder builder) {
		final String encoding = alleleMap.get(allele);
		if (encoding == null)
			throw new RuntimeException("Allele " + allele
					+ " is not an allele in the variant context");
