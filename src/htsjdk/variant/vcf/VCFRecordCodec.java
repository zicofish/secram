...
/**
 * Writes VariantContext instances to an OutputStream without headers or
 * metadata. For use with SortingCollection ONLY.
 */
public class VCFRecordCodec implements SortingCollection.Codec<VariantContext> {
	...
	public VCFRecordCodec(final VCFHeader header) {
		...
		// Explicitly set the version because it's not available in the header
		// itself.
	...
	public VariantContext decode() {
		try {
			final String line;
			return ((line = inputReader.readLine()) != null) ? this.vcfDecoder
					.decode(line) : null;
		} catch (final IOException ioe) {
			throw new RuntimeIOException(
					"Could not decode/read a VCF record for a sorting collection: "
							+ ioe.getMessage(), ioe);
		}
	}
