...
/**
 * this is an implementation of a Map of JexlVCMatchExp to true or false values.
 * It lazy initializes each value as requested to save as much processing time
 * as possible.
 *
 * Compatible with JEXL 1.1 (this code will be easier if we move to 2.0, all of
 * the functionality can go into the JexlContext's get()
 *
 */
...
class JEXLMap implements Map<JexlVCMatchExp, Boolean> {
	...
	// our mapping from JEXLVCMatchExp to Booleans, which will be set to NULL
	// for previously uncached JexlVCMatchExp
	private Map<JexlVCMatchExp, Boolean> jexl;

	public JEXLMap(Collection<JexlVCMatchExp> jexlCollection,
			VariantContext vc, Genotype g) {
		this.vc = vc;
		this.g = g;
	...
	/**
	 * create the internal JexlContext, only when required. This code is where
	 * new JEXL context variables should get added.
	 *
	 */
	private void createContext() {
		if (g == null) {
			...
			if (vc != null) {
				// create a mapping of what we know about the variant context,
				// its Chromosome, positions, etc.
				...
				// TODO -- implement me when we figure out a good way to
				// represent this
				...
				infoMap.put(VCFConstants.GENOTYPE_FILTER_KEY,
						g.isFiltered() ? g.getFilters() : "PASS");

				infoMap.put(VCFConstants.GENOTYPE_QUALITY_KEY, g.getGQ());
				if (g.hasDP())
					infoMap.put(VCFConstants.DEPTH_KEY, g.getDP());
				for (Entry<String, Object> e : g.getExtendedAttributes()
						.entrySet()) {
					if (e.getValue() != null
							&& !e.getValue().equals(
			...
			// create the internal context that we can evaluate expressions
			// against
	...
	/**
	 * get all the values of the map. This is an expensive call, since it
	 * evaluates all keys that haven't been evaluated yet. This is fine if you
	 * truely want all the keys, but if you only want a portion, or know the
	 * keys you want, you would be better off using get() to get them by name.
	 * 
	 * @return a collection of boolean values, representing the results of all
	 *         the variants evaluated
	 */
	@Override
	...
	/**
	 * evaulate a JexlVCMatchExp's expression, given the current context (and
	 * setup the context if it's null)
	 * 
	 * @param exp
	 *            the JexlVCMatchExp to evaluate
	 */
	private void evaluateExpression(JexlVCMatchExp exp) {
		// if the context is null, we need to create it to evaluate the JEXL
		// expression
		...
		try {
			...
			// if exception happens because variable is undefined (i.e. field in
			// expression is not present), evaluate to FALSE
			// todo - might be safer if we explicitly checked for an exception
			// type, but Apache's API doesn't seem to have that ability
			if (e.getMessage().contains("undefined variable"))
				jexl.put(exp, false);
			else
				throw new IllegalArgumentException(
						String.format(
								"Invalid JEXL expression detected for %s with message %s",
								exp.name, e.getMessage()));
		}
	}

	/**
	 * helper function: adds the list of attributes to the information map we're
	 * building
	 * 
	 * @param infoMap
	 *            the map
	 ...
	 *            the attributes
	 */
	private static void addAttributesToMap(Map<String, Object> infoMap,
	...
	// this doesn't make much sense to implement, boolean doesn't offer too much
	// variety to deal
	// with evaluating every key in the internal map.
	@Override
	public boolean containsValue(Object o) {
		throw new UnsupportedOperationException(
				"containsValue() not supported on a JEXLMap");
	}
	...
	public Boolean remove(Object o) {
		throw new UnsupportedOperationException(
				"remove() not supported on a JEXLMap");
	}
